...
#### 功能增强
- 新增 可配置老板键功能：将固定的 Ctrl+~ 隐藏窗口快捷键改为可在 `更多设置` 中配置，支持 Ctrl/Alt 组合键 + ~、1、2 共六种组合；切换时实时重新注册热键，若被其他程序占用则提示并恢复原绑定
- 新增 美股/港股独立交易时段刷新：新增按美国/香港交易时段判断逻辑（IsUSMarketOpen/IsHKMarketOpen），使美股、港股在非 A 股交易时段也能正常刷新，纯 A 股列表行为保持不变
- 新增 代码后缀归一化匹配：自动去除交易所后缀（如 AAPL.OQ、00700.HK），修复配置代码与接口返回代码不匹配导致无法刷新的问题

#### 修复
- 修复 ETF 小数位显示错误：原按名称是否以 "ETF" 结尾判断是否显示 3 位小数，导致华宝添益等名称不带 ETF 后缀的货币/债券/黄金/跨境 ETF 被误判为普通股票、金额仅显示 2 位。现改为优先按代码前缀（沪市 51/56/58、深市 15/16）判定，名称包含 ETF 兜底，且持仓买入价也跟随显示 3 位

#### 配置项新增
- 新增 BossKey：老板键组合配置（默认 Ctrl+~）
- 配置版本升级至 v8
